...
describe('agent artifact desktop download handoff', () => {
  it('prevents popup navigation and delegates to Electron when desktop bridge exists', async () => {
    ...
    expect(downloadAgentArtifact).toHaveBeenCalledWith({ url: 'http://127.0.0.1:8790/api/jobs/job-1/artifacts/a.zip?ownerToken=owner-token-0001' });
  });

  it('leaves normal web navigation alone without desktop bridge', async () => {
    const preventDefault = vi.fn();

    await expect(handleAgentArtifactDownloadClick({ preventDefault }, '/api/jobs/job-1/artifacts/a.zip', undefined))
  ...
  it('surfaces desktop handoff failures', async () => {
    ...
    await expect(handleAgentArtifactDownloadClick({ preventDefault }, 'https://evil.example/a.zip', { downloadAgentArtifact }))
