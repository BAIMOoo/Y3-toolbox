...
导入包含 `archive_diff` 的 CSV 后，工作台会生成时间线、变动列表、快照对比和状态栏统计。你可以按时间范围、存档键、变动类型或关键词筛选，下载整理后的 CSV，也可以基于目标时间生成可审核的存档回退 JSON 输入。
...
- **生成回退输入**：对单个玩家的变动日志，按目标时间后第一次变动的旧值推算可证明的槽位片段，并导出 JSON 供人工审核或后续工具使用；应用本身不会写回存档。
...
`技术问答` 是独立于 Agent 任务中心的 Y3 Editor 2.0 问答工作区，覆盖 `ECA / 编辑器` 和 `Lua / y3-lualib`。每次回答都会由服务端模型结合受治理的证据生成，不会把检索结果直接当作答案。工作区支持引用来源、诊断附件、取消与重试，并在服务不可用时保留当前草稿和会话历史。
...
| Agent 任务 | 提交固定范围的共享服务任务并查看结果 | 拉取日志或导出资源 |
| 技术问答 | 询问 Y3 Editor 2.0 的 ECA 或 Lua 技术问题 | 查用法、定位问题或分析明确上传的诊断材料 |
