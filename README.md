...
`Y3 工具箱` 是一个面向 Y3 地图作者、玩法脚本作者和技术排查人员的 Windows 桌面工具。它把本地存档诊断、任务服务、大厅配置和反馈入口放在同一个工作区里，目标是让“查一份日志、看一个 Archive、提交一项受控任务、改一份大厅配置”这些高频操作都能在桌面端完成。

应用以 Electron 桌面端为主线。本地 CSV / Archive / 大厅配置能力不依赖在线服务；Agent 任务中心和反馈功能会连接各自独立的服务边界。
...
- 拉取存档变动日志：单次时间范围最长 15 天；存档日志只保留最近约 30 天，更早的记录已经拉取不到。
- 拉取不同步日志。
- 导出 kkres 高分辨率图片资源。
